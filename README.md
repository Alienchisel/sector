...
- **Navigation** — a clickable breadcrumb; click the bar's empty space (or
  `Alt+D` / `Ctrl+L` / `F4`) to type a path instead. Plus **Back/Forward/Up** (`Alt+←` or `Backspace` / `Alt+→` / `Alt+↑`)
...
- **Details** panel (`Alt+Enter` opens, `Alt+Shift+P` toggles, or the toolbar toggle) — path, sizes, dates,
...
- **Refresh** with `F5` / `Ctrl+R` (`F11` toggles fullscreen); the current folder also refreshes when
